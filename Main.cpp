...
void main(int argc, char* argv[]) {
	...
	if (ProcRank == 0) {
		//rgb_image = stbi_load("file.png", &width, &height, &bpp, STBI_grey);
		width = atoi(argv[1]);
		height = atoi(argv[2]);
	...
	/*if (ProcRank != 0) {
		MPI_Sendrecv(recvbuf + offset, width, MPI_UNSIGNED_CHAR,
			ProcRank - 1, 0, recvbuf, width, MPI_UNSIGNED_CHAR,
			ProcRank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	}
	if (ProcRank != psize - 1) {
		MPI_Sendrecv(recvbuf + offset + elem_proc[ProcRank] - width, width,
			MPI_UNSIGNED_CHAR, ProcRank + 1, 0,
			recvbuf + offset + elem_proc[ProcRank], width, MPI_UNSIGNED_CHAR,
			ProcRank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	}
*/
	if (ProcRank != psize - 1) {
